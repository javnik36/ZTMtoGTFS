    ...
    for line in file:
        ...
        if start != None:
            ...
            if linia != None:
                ...
                # RUTE-ID,,NR-LiNII,,,TYPE,URL,,
                route_id = nr_linii
                url = "http://www.ztm.waw.pl/rozklad_nowy.php?c=182&l=1&q=" + \
                    nr_linii
                save_route.write(
                    route_id + koma * 2 + nr_linii + koma * 3 + route_type + koma + url + koma * 2 + '\n')

            if inWK == True:
                # bez P-godz. odjzdu, z P-godz. przyjazdu
                tekst_sam = re.compile(
                    "(.{17})\s+(\d{6})\s(\w{2})\s+(\d+\.\d+)")
                tekst_zP = re.compile(
                    "(.{17})\s+(\d{6})\s(\w{2})\s+(\d+\.\d+)\s+\P")
                ...
                if macz_sam != None:
                    ...
                    save_times.write(print_trip_id + koma + arrival + koma +
                                     departure + koma + stop + koma + str(iterator) + koma * 4 + '\n')
                    ...
                    if trip_id in temp:
                        ...
                        temp.append(trip_id)
                        save_trips.write(
                            route_id + koma + service_id + koma + print_trip_id + koma * 7 + '\n')
                    ...
                    save_times.write(
                        print_trip_id + koma + arrival + koma * 2 + stop + koma + str(iterator) + koma * 4 + '\n')
